...
type Resource struct {
	Code      uint64 //Resourcecode
	Id        int32  //(option)果实需要记录id
	Timestamp int64  //(option)记录的时刻
}

//State 物品状态(绑定于地块)
type State interface {
	GetSite() uint32    //Sitecode
	GetBitag() uint32   //StateBitag
	GetCutime() int64   //cut time
	GetEx() int32       //城堡记录经验值；计时工人记录剩余工作时间
	GetAwards() []int32 //awards after open
}

//Itemdist item distributed
...
type Farm interface {
	...
	DoUnlock(plot uint8) bool //unlock plot
	io.Closer
}
